...
struct asyncx_callbacks {
	...
	int		(*intercept_do_page_fault)(struct vm_area_struct *vma,
 ...
 */
static inline int
asyncx_intercept_do_page_fault(struct vm_area_struct *vma,
				 unsigned long address,
				 unsigned int flags)
{
	return acb_live.intercept_do_page_fault(vma, address, flags);
}
